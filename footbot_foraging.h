...
#include <chrono>   // For file handle?
#include <fstream>  // For read and write to the file?
/* FOr get ticks number */
...
public:
   ...
   //I make a new one with content, that close the file so needed to delete the one above
   virtual ~CFootBotForaging();
...
private:
   ...
   /* Here we will handle the collision
   *
   */
   CVector2 handleCollision(CVector2 cDiffusionVector);
...
private:
   ...
   /*
      This enum contain all the posibale behaviors of what a robot can 
      do in a collision.
   */
   ...
   // This tells us if the robot has ended his last collision
